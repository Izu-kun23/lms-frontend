...
export function CreateContentModal({
  ...
  onOpenChange,
  onContentCreated,
  existingContentsCount = 0,
}: CreateContentModalProps) {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [formData, setFormData] = useState({
    kind: "TEXT" as ContentType,
    text: "",
    mediaUrl: "",
    index: existingContentsCount + 1,
    metadata: "",
  })
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // For LIVE - might not need mediaUrl, but can include metadata
      if (formData.kind === "LIVE") {
        // LIVE content might not require mediaUrl
        if (formData.mediaUrl.trim()) {
          requestData.mediaUrl = formData.mediaUrl.trim()
        }
      }
      ...
      console.log("Creating content with payload:", requestData)
      const newContent = await apiClient.createLectureContent(lectureId, requestData)
      ...
      <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
          ...
          <SheetDescription>
            {lectureTitle && (
              <span className="block mb-2">
                Add content to <strong>{lectureTitle}</strong>
              </span>
            )}
            Content can be text, video, audio, files, or live sessions. Choose the appropriate type and provide the necessary information.
          </SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          <div className="space-y-4">
            ...
            {(formData.kind === "VIDEO" || formData.kind === "AUDIO" || formData.kind === "FILE") && (
              <div>
          ...
          <div className="flex gap-3 pt-4">
            <Button
              type="submit"
              disabled={isSubmitting || (formData.kind === "TEXT" && !formData.text.trim()) || (formData.kind !== "TEXT" && !formData.mediaUrl.trim())}
